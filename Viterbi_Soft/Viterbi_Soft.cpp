...
double state_table[state_num+5][state_type];//state table, the size should be defined yourself
...
void main()
{
	...
	double progress;
	//fstream
	ofstream fout("data_Viterbi_Soft");
	///////////////////////////////////////////////////
...
int edge_id[state_num+5][state_type];
void decoder()
{
	//statetable();
	int BPSK_symbol[2];
	...
	for (int i = 0; i<codeword_length; i += 2) //i+= code_rate (rewrite)
	{
		...
		{
			// (rewrite)
			double d1 = (codeword_x1 - BPSK_symbol[(path[j].OUT>>1) & 1]) * (codeword_x1 - BPSK_symbol[(path[j].OUT >> 1) & 1]) + codeword_y1 * codeword_y1;
			double d2 = (codeword_x2 - BPSK_symbol[path[j].OUT & 1]) * (codeword_x2 - BPSK_symbol[path[j].OUT & 1]) + codeword_y2 * codeword_y2;
			// Need for sqrt() ???
			double dis = d1 + d2;
